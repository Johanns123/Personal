...
extern uint32_t vbat[2];

extern uint8_t rx_data;		//variáveis da UART
...
unsigned char max_timer1, max_timer2, max_timer3_ms, max_timer_3, max_timer4, max_timer5;   //vari�veis de controle de temporiza��o
...
{
	//verificará a tensão da bateria
}
...
void setup_logica() /*Fun��o que passa ponteiros para fun��es como par�m*/
{
    ...
    max_timer4 = 10, max_timer5 = 10;   //bases de tempo ciadas com uma base m�nima de 100us
}
...
void parada()
{
    ...
    sensors_le_marcadores();//faz a leitura dos sensores laterais e verifica se � uma curva, cruzamento ou parada
...
void count_pulsesD()
{
    ...
    bool Lstate = 0;//(tst_bit(leitura_outros_sensores, encoder_C1D) >> encoder_C1D); //vari�vel de leitura de um dos pinos do encoderD

    if (!Encoder_C1Last && Lstate) {
        //Verifica se Encoder_C1Last � falso e Lstate � verdadeiro
        bool val = 0;//tst_bit(leitura_outros_sensores, encoder_C2D >> encoder_C2D); //Vari�vel de leitura do segundo pino do encoderD
...
void count_pulsesE()
{
    ...
    bool Lstate = 0;// (tst_bit(leitura_sensores, encoder_C1E) >> encoder_C1E); //vari�vel de leitura de um dos pinos do encoderD

    if (!Encoder_C1Last && Lstate)
    { //Verifica se Encoder_C1Last � falso e Lstate � verdadeiro
        bool val = 0;//(tst_bit(leitura_sensores, encoder_C2E) >> encoder_C2E); //Vari�vel de leitura do segundo pino do encoderD
...
void millis(void)//cron�metro de base de 1ms
{
...
{
    //ftimer3 � uma fun��o com uma base de tempo de ms dentro dela e que novas fun��es s�o chamadas numa escala de ms
